...
void describe_znak(Mat Fragment,double min_x,double max_x, double max_val) {
	...
	for (int n = 0; n < NUMBER; n++) {
		...
		if (a[1] > 100000 && a[1] > a[0] && ((a[1] - 10000) / 10 > (a[0] - 10000) / 10 || a[1] > a[0])) {


			cout << "Autobus line" << endl;
		}
...
void find_perehod(Mat FragmentC) {
	...
	resize(perehod, perehod, Size(256, 128));
...
void obrabotka(string path) {
	...
	while (1) {
		...
		thread o(find_five_max_contours,contours,FragmentC);
		thread oo(find_perehod,FragmentC);
		o.detach();
		oo.detach();
...
int main()
{
	...
	get_etalone_images();
		...
		thread thr1(obrabotka,path);
		thread thr2(show_video,path);
		thr1.join();
		thr2.join();
...
int main()
{
	...
	if (!videoSource.open("vodeo5.mp4"))
	{
		std::cout << "Video not found" << std::endl;
